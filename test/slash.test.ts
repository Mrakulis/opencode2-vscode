...
describe("filterSlashEntries", () => {
  ...
  it("matches case-insensitively on name", () => {
    ...
    assert.deepEqual(
      filterSlashEntries(entries, "init").map((e) => e.name),
      ["init"],
    );
  });
  ...
  it("filters by kind", () => {
    assert.deepEqual(
      filterSlashEntries(entries, "", "command").map((e) => e.name),
      ["review", "init"],
    );
    assert.deepEqual(
      filterSlashEntries(entries, "", "skill").map((e) => e.name),
      ["deep-research", "testgen"],
    );
    assert.equal(filterSlashEntries(entries, "", "all").length, 4);
    // query + kind compose
    assert.deepEqual(
      filterSlashEntries(entries, "re", "skill").map((e) => e.name),
      ["deep-research"],
    );
  ...
  it("routes local (gui) entries to their own filter", () => {
    ...
    assert.equal(
      filterSlashEntries(withGui, "", "command").some((e) => e.local),
      false,
    );
  ...
  it("caps results at 40", () => {
    const many: SlashEntry[] = Array.from({ length: 60 }, (_, i) => ({
      kind: "command",
      name: `cmd${i}`,
    }));
