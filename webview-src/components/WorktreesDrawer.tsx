...
export function WorktreesDrawer({ onClose }: { onClose(): void }) {
  ...
  const load = useCallback(async () => {
    try {
      let pid = projectID;
      if (!pid) {
        const cur = await rpc.call<Record<string, unknown> | undefined>("project.current");
      ...
      const list = await rpc.call<Array<Record<string, unknown>>>("worktree.list", { projectID: pid });
      setRows(
        list.map((r) => ({
          name: typeof r.name === "string" ? r.name : undefined,
          directory: typeof r.directory === "string" ? r.directory : typeof r.path === "string" ? r.path : undefined,
  ...
  return (
    <div className="drawer-backdrop" onClick={onClose}>
      <aside className="drawer" onClick={(e) => e.stopPropagation()} aria-label="Worktrees">
        ...
        <div className="drawer-body">
          <div style={{ display: "flex", gap: "6px", marginBottom: "10px" }}>
            <input className="search" placeholder="new worktree name" value={name} onChange={(e) => setName(e.target.value)} />
            <button type="button" className="primary" disabled={!projectID} onClick={() => void create()}>Add</button>
          </div>
          {error && <div className="composer-error">{error}</div>}
          {rows.length === 0 && !error && <div className="drawer-empty">No worktrees yet.</div>}
          {rows.map((r, i) => (
            <div key={(r.directory ?? r.name ?? "") + i} className="session-row">
              <div style={{ flex: 1, overflow: "hidden" }}>
                <div className="session-title">{r.name ?? r.directory ?? "(unnamed)"}</div>
                <div className="session-meta">{r.branch ?? ""} {r.directory ?? ""}</div>
              </div>
              <button
                type="button"
                className="rowdel"
