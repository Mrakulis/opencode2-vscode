...
- Context % must prefer session-level `active.tokens` over the last assistant message's tokens - post-compact the message still carries pre-compaction counts until the next turn (`session.usage.recorded` keeps session tokens fresh).
