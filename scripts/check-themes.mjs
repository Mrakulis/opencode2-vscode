...
const baseTokens = [...rootBlock[2].matchAll(/--oc2-[a-z0-9-]+\s*:/g)].map((m) => m[0].trim())
...
const blocks = [...css.matchAll(/:root\[data-theme="([a-z]+)"\] \{([\s\S]*?)\n\}/g)];
let failures = 0;
for (const [, id, body] of blocks) {
  const defined = new Set([...body.matchAll(/(--oc2-[a-z0-9-]+)\s*:/g)].map((m) => m[1]));
  const missing = baseTokens.filter((t) => !defined.has(t.replace(/\s*:$/, "")));
  if (missing.length > 0) {
    console.error(`FAIL ${id}: missing ${missing.length} token(s): ${missing.join(", ")}`);
    failures++;
  } else {
